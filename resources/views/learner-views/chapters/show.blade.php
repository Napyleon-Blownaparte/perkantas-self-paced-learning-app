<x-app-layout>
    <div class="flex h-screen">
        ...
        <div id="main-content" class="flex-1 ml-[16em] transition-all duration-300 ease-in-out overflow-visible">
            ...
            <!-- Main Content -->
            <div>
                <button id="toggleSidebar" class="fixed z-10 bg-white text-black font-extrabold py-3 px-1 shadow-xl rounded-tr-3xl rounded-br-3xl top-[3.5em] left-[16em] transform transition-all duration-300 ease-in-out">
                ...
                @foreach ($chapter->materials as $index => $material)
                    ...
                    @else
                        <x-section-reverse class="px-20" title="{{ $material->title }}" content="{{ $material->content }}"
                            imgSrc="{{ $material->image }}" videoSrc="{{ $material->video }}" />
                    @endif
                @endforeach
    ...
    toggleSidebarButton.addEventListener('click', function() {
        ...
        if (sidebar.classList.contains('-translate-x-full')) {
            ...
            toggleSidebarButton.classList.add('left-[16em]'); // Move the button to the right when sidebar is shown
