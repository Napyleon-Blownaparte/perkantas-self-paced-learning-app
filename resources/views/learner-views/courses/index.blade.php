<x-app-layout>
    <div class="flex-1 p-8 ml-16 mb-8 sm:ml-4 sm:p-4 md:ml-4 md:p-6">
        <h1 class="text-4xl font-bold mb-4 sm:text-2xl md:text-3xl">Courses Available</h1>

        <!-- Filter dropdown -->
        @auth
            <div class="mb-4 flex items-center sm:flex-col sm:items-start md:flex-row">
                <label for="enrollment-status" class="mr-2 text-lg font-medium sm:mb-2 sm:text-sm">Filter by Enrollment Status:</label>
                <form method="GET" action="{{ url()->current() }}" class="w-full">
                    <select name="status" id="enrollment-status" 
                        class="border rounded px-3 py-2 w-full sm:w-full md:w-1/2"
        ...
        @if ($courses->isEmpty())
            <div class="mb-64">
                <svg class="svg-icon text-slate-400 m-auto translate-y-20 sm:w-40 sm:h-40"
                ...
                <p class="text-slate-500 text-center translate-y-[6em] sm:translate-y-[3em] sm:text-sm">Pencarian tidak ditemukan :(</p>
            </div>
        @else
            <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                @foreach ($courses as $course)
                    <div class="flex flex-col items-center bg-white shadow-md rounded-lg overflow-hidden mb-6 sm:w-full">
            ...
            <p class="text-black mt-8 sm:mt-6 text-sm">
